...
namespace Dumplings.Stats
{
    public class Statista
    {
        ...
        public async Task GetTx(ExtPubKey[] extPubKeys)
        {
            DateTimeOffset? lastCoinJoinTime = null;
            StringBuilder sb = new();
            var scripts = Constants.WasabiCoordScripts.ToHashSet();
            ...
            List<VerboseBlockInfo> blocks = new()
            {
                await Rpc.GetVerboseBlockAsync(795179, safe: false).ConfigureAwait(false),
                await Rpc.GetVerboseBlockAsync(795511, safe: false).ConfigureAwait(false),
                await Rpc.GetVerboseBlockAsync(796471, safe: false).ConfigureAwait(false),
            };
